...
int Efficiency(string process, vector<string> periods = {"LHC16r", "LHC16s"}, Double_t mMin = 2.9, Double_t mMax = 3.2, Double_t rapMin = 2.5, Double_t rapMax = 4.0) {
    ...
    for (int l = 0; l < (int)periods.size(); l++) {
        ...
        cout << "Events in tree" << endl;
        cout << "Generated: " << nGen << endl;
        cout << "Reconstructed (2 good tracks + trigger): " << nRec << endl;
        cout << endl;
        ...
        cutMc += rapCutMc;
        GetTotalEff(process, periods[l], cut, cutMc, nGen, nRec);
        cout << "+ within the right rapidity cut " << rapMin << " < y < " << rapMax << endl;
        cout << "Generated: " << nGen << endl;
        cout << "Reconstructed: " << nRec << endl;
        cout << endl;
        ...
        cutMc += massCutMc;
        GetTotalEff(process, periods[l], cut, cutMc, nGen, nRec);
        cout << "+ with " << mMin << " < M < " << mMax << endl;
        cout << "Generated: " << nGen << endl;
        cout << "Reconstructed: " << nRec << endl;
        cout << endl;
        
        float eff = (float)nRec/nGen;
        //cout << "So Efficiency = (" << eff*100 << " \u00b1 " << TMath::Sqrt(eff*(1-eff)/(float)nGen)*100 << ") %" << endl;
        cout << "So Efficiency = " << eff << " \u00b1 " << TMath::Sqrt(eff*(1-eff)/(float)nGen)  << endl;
        cout << endl;
        
        /*
        // Fifth: histogram as a function of pt
        Int_t nBins = 20;
        Double_t ptMin = 0, ptMax = 2;
