...
const Double_t gxmin = 9., gymin = 5., gxmax = 2100., gymax = 1300.;
...
class alice {
  ...
  static const Int_t ALICE_n = 11;      // fw     fw   fw    sfw     sfw    mid      mid   sfw      fw
  const Double_t ALICE_W[ALICE_n] = {24.1, 30.9, 39.6, 50.4,  73.1,  129.9,  193.3,  391.2,  706., 33.0, 48.0};
  const Double_t ALICE_sig[ALICE_n] = {27.6, 34.7, 38.5, 41.9,  61.9,  101.4,  126.2,  194.5,  284., 33.7, 48.5};
  const Double_t ALICE_sta[ALICE_n] = { 3.6,  3.1,  5.6,  8.6,  10.8,    8.1,   10.0,   27.4,   36, 3.0, 5.2};
  // model A, considers MUON and trigger efficiencyes uncorrelated
  static const Int_t nUncA = 10; //  sources of uncorrelated error
  const Double_t ALICE_2013_unc_A[nUncA][ALICE_n] = // relative uncertainty!
  {
    {0, 0, 0, 0.057, 0.012, 0.009, 0.008, 0.033, 0, 0, 0}, // TPC track selection
    {0, 0, 0, 0, 0, 0.013, 0.006, 0, 0, 0, 0}, // PID efficiency
    {0.04, 0.04, 0.04, 0.02, 0.02, 0, 0, 0.03, 0.06, 0.01, 0.01}, // muon track eff
    {0.01, 0.01, 0.01, 0.005, 0.005, 0, 0, 0.005, 0.01, 0.01, 0.01}, // muon matching eff
    {0.028, 0.028, 0.028, 0.02, 0.02, 0, 0 , 0.016, 0.032, 0.026, 0.026}, // muon trig eff
    {0, 0, 0, 0, 0, 0.101, 0.101, 0, 0, 0, 0}, // central trigger eff
    {0, 0, 0, 0, 0, 0, 0, 0.034, 0.035, 0.026, 0.026}, // V0C efficiency
    {0.066, 0.037, 0.08, 0.036, 0.022, 0.021, 0.019, 0.03, 0.06, 0.036, 0.040}, // signal extraction, last number should be -0,+0.06
    {0.031, 0.02, 0.02, 0.013, 0.01, 0.01, 0.01, 0.014, 0.031, 0.011, 0.011}, // feed-down
    //lumi at mid-rapidity by quadratic average:
    //sqrt((3.3%*2.1)**2 + (3%*4.8)**2) / (2.1 + 4.8) = 2.3%
    {0.033, 0.033, 0.033, 0.033, 0.033, 0.023, 0.023, 0.03, 0.03, 0, 0} // lumi uncorrelated
  };
  Double_t ALICE_2013_unc[ALICE_n]; // to store total uncorrelated uncertainty
  static const Int_t nCorA = 3; // sources of correlated error
  const Double_t ALICE_2013_corr_A[nCorA][ALICE_n] = // relative uncertainty!
  {
    {0.016, 0.016, 0.016, 0.016, 0.016, 0.016, 0.016, 0.016, 0.016, 0.018, 0.018}, // lumi
    {0.006, 0.006, 0.006, 0.006, 0.006, 0.004, 0.004, 0.006, 0.006, 0.006, 0.006}, // br
    {0.02, 0.02, 0.02, 0.027, 0.035, 0.021, 0.021, 0.012, 0.02, 0.021, 0.021}//, // v0a veto
    //{0, 0, 0, 0, 0, 0.09, 0.09, 0, 0} // TOF trigger, Eur. Phys. J. C (2013) 73:2617, now in central trigger eff
  };
  Double_t ALICE_2013_tot[ALICE_n]; // to store total error
  Double_t ALICE_statsys[ALICE_n]; // to store stat + total sys error for the plot
...
alice::alice(Int_t oo) : opt(oo), Nval(-1), Dval(-1), fitGraph(0) {
  ...
  for(Int_t i=0; i<ALICE_n; i++) {ALICE_2013_unc[i]=0.; ALICE_2013_tot[i]=0.; ALICE_statsys[i]=0.;}
...
void alice::TotalErrorA()
{
  ...
  for(Int_t j=0;j<ALICE_n;j++) {
    ...
    ALICE_2013_tot[j]=TMath::Sqrt(tot)*ALICE_sig[j];
...
// quadratic sum of statistical uncertainty and total systematic uncertainty, to be used for the plot
  for(Int_t i=0; i<ALICE_n; i++) {
    Double_t err = 0.;
    err = TMath::Power(ALICE_sta[i], 2) + TMath::Power(ALICE_2013_tot[i], 2);
    ALICE_statsys[i] = TMath::Sqrt(err);
  }
}
TGraph* alice::GetShade(Double_t N, Double_t d, Double_t cov00, Double_t cov11, Double_t cov01)
{
    Double_t protonMass = 0.938272;     // GeV
    Double_t jpsiMass = 3.096916;        // GeV
// Define graph with error band from results of the fit
  ...
  for (Int_t i=0;i<=nw;i++){
    ...
    //Double_t y = N*pow(w/90,d);
      Double_t y = N*pow(w/90,d)*pow(1-pow((protonMass+jpsiMass)/w,2),2);
    Double_t dydN = y/N;
...
// Eur.Phys.J. C63 (2009) 625-678, section 9, eq (31)
  ...
  Double_t chi2 = 0;
    ...
    Double_t jpsiMass = 3.096916;        // GeV
  for (Int_t i=0; i<ALICE_n;i++) {
    Double_t mu_i = ALICE_sig[i];
    //Double_t m_i = par[0]*TMath::Power(ALICE_W[i]/90,par[1]);
      Double_t m_i = par[0]*TMath::Power(ALICE_W[i]/90,par[1])*TMath::Power(1-TMath::Power((protonMass+jpsiMass)/ALICE_W[i],2),2);
    Double_t Sij = 0;
    for(Int_t j=0;j<nCorA;j++) Sij += m_i*bj[j]*ALICE_2013_corr_A[j][i];
    Double_t d_stat_i =  ALICE_sta[i]/ALICE_sig[i];
...
TGraphErrors *alice::getForward() {
  //forward data
  const Int_t np = 4;
  const Double_t w[np] = {ALICE_W[0], ALICE_W[1], ALICE_W[2], ALICE_W[8]};
  const Double_t sig[np] = {ALICE_sig[0], ALICE_sig[1], ALICE_sig[2], ALICE_sig[8]};
  const Double_t err[np] = {ALICE_statsys[0], ALICE_statsys[1], ALICE_statsys[2], ALICE_statsys[8]};
...
TGraphErrors *alice::getForwardEightTeV() {
    ...
    const Double_t sig[np] = {  33.7, 48.5};
    //const Double_t err[np] = {  2.500,                      4.700                      };
    const Double_t err[np] = {   3.0,                      5.2                      };
...
TGraphErrors *alice::getNewData() {
  ...
  const Int_t np = 5;
  const Double_t *w = &ALICE_W[3];
  const Double_t *sig = &ALICE_sig[3];
  const Double_t *err = &ALICE_statsys[3];
/*
...
void alice::printData() {
  ...
  const Int_t np = 9;
  const Double_t *w = ALICE_W;
  const Double_t *sig = ALICE_sig;
  const Double_t *err = ALICE_statsys;
...
TGraphErrors* read_cct(const char* fileName){
    // read H1 data from https://hep.fjfi.cvut.cz/projects/hotspots/paper-1/Fig3_exc.txt
    ifstream f;
    f.open(fileName);
    string b;
    Double_t wav, sig;
...
// LHCb @13 TeV
TGraphErrors* read_lhcb_pos_new() {
    //forward data
    const Int_t np = 10;
    
    const Double_t w[np]   = {581, 658, 746, 845, 958, 1085, 1230, 1394, 1579, 1790};
    const Double_t statErr[np] = {9.2, 6.0, 5.0, 4.5, 4.3, 4.3, 4.4, 4.8, 5.7, 8.5};
    const Double_t systErr[np] = {4.3, 2.7, 3.1, 2.7, 2.6, 2.6, 2.6, 2.6, 2.7, 2.8};
    const Double_t lumiErr = 3.9;
    
    const Double_t sig[np] = {291, 335, 321, 339, 358, 395, 403, 403, 456, 524};
    Double_t err[np] = {};

    for (int i = 0; i < np; i++) {err[i] = sig[i]*TMath::Sqrt( (statErr[i]/100)*(statErr[i]/100) + (systErr[i]/100)*(systErr[i]/100) + (lumiErr/100)* (lumiErr/100)) ;}
    
    TGraphErrors *gr = new TGraphErrors(np, w, sig, nullptr, err);
...
// LHCb @13 TeV
TGraphErrors* read_lhcb_neg_new() {
    //forward data
    const Int_t np = 10;
    
    const Double_t w[np]   = {69.4, 61.2, 54.0, 47.7, 42.1, 37.1, 32.8, 28.9, 25.5, 22.5};
    const Double_t statErr[np] = {9.2, 6.0, 5.0, 4.5, 4.3, 4.3, 4.4, 4.8, 5.7, 8.5};
    const Double_t systErr[np] = {4.3, 2.7, 3.1, 2.7, 2.6, 2.6, 2.6, 2.6, 2.7, 2.8};
    const Double_t lumiErr = 3.9;
    
    const Double_t sig[np] = {68.0, 62.6, 57.6, 52.9, 48.7, 37.1, 32.8, 28.9, 25.5, 22.5};
    Double_t err[np] = {};
...
// H1 2005 from https://arxiv.org/pdf/hep-ex/0510016.pdf
TGraphErrors* read_h1_2005() {
    //forward data
    const Int_t np = 18;
    
    const Double_t w[np]   = {44.8, 54.8, 64.8, 74.8, 84.9, 94.9, 104.9, 119.5, 144.1, 144.9, 162.5, 177.3, 194.8, 219.6, 219.6, 244.8, 267.2, 292.3};
    const Double_t statErr[np] = {2.4, 2.3, 2.8, 3.2, 3.4, 3.7, 4.0, 3.5, 4.4, 6.6, 8, 8, 10, 14, 10, 13, 13, 19};
    const Double_t systErr[np] = {4.0, 4.3, 5.3, 5.5, 6.4, 6.9, 7.3, 8.1, 8.7, 9.6, 11, 12, 15, 25, 18, 17, 18, 23};
    
    const Double_t sig[np] = {46.0, 48.5, 59.7, 62.7, 72.6, 78.6, 82.6, 91.5, 98.3, 98.6, 114, 126, 143, 187, 133, 171, 173, 194};
    Double_t err[np] = {};
    
    for (int i = 0; i < np; i++) {err[i] = sig[i]*TMath::Sqrt( (statErr[i]/sig[i])*(statErr[i]/sig[i]) + (systErr[i]/sig[i])*(systErr[i]/sig[i]) ) ;}
...
// E401 and E516 and E687
TGraphAsymmErrors* read_E687() {
    /*
     TFile* f = new TFile("HEPData-ins359356-v1-Table_2.root", "read");
     TGraphErrors* g = (TGraphErrors*)f->Get("Table 2/Graph1D_y1");
     */
    
    const int np = 6;
    const Double_t ep[np]   = {121.0, 177.0, 223.0, 272.0, 324.0, 374.0};
    Double_t w[np]   = {};
    Double_t wlow = 15.096;
    Double_t whigh = 26.505;
    for (int i = 0; i < np; i++) {
        w[i] = wlow+ (whigh-wlow)/(ep[np-1]-ep[0])*(ep[i]-ep[0]);
    }
    
    const Double_t sig[np] = {12.3, 16.6, 14.0, 17.9, 26.7, 19.9};
    const Double_t statErr[np] = {3.0, 3.8, 3.4, 4.2, 7.3, 10.9};
    const Double_t systErrPos[np] = {1.1, 1.5, 1.3, 1.6, 2.4, 1.8};
    const Double_t systErrNeg[np] = {2.2, 3.0, 2.6, 3.2, 4.8, 3.6};
    
    Double_t errPos[np] = {};
    Double_t errNeg[np] = {};
    
    for (int i = 0; i < np; i++) {
        errPos[i] = sig[i]*TMath::Sqrt( (statErr[i]/sig[i])*(statErr[i]/sig[i]) + (systErrPos[i]/sig[i])*(systErrPos[i]/sig[i]) ) ;
        errNeg[i] = sig[i]*TMath::Sqrt( (statErr[i]/sig[i])*(statErr[i]/sig[i]) + (systErrNeg[i]/sig[i])*(systErrNeg[i]/sig[i]) ) ;
    }
    TGraphAsymmErrors* gr = new TGraphAsymmErrors(np, w, sig, NULL, NULL, errNeg, errPos);
    /*
     const int np = g->GetN();
     const Double_t* xvalues = g->GetX();
     const Double_t* yvalues = g->GetY();
     const Double_t* yerrors = g->GetEY();
     
     TGraphErrors *gr = new TGraphErrors(np, *xvalues, *yvalues, nullptr, *yerrors);
     */
    //gr->GetXaxis()->SetRangeUser(15.096, 26.505);
    return gr;
}
...
TGraphErrors* read_E516() {
    ...
    const int np = 1;
     const Double_t w[np]   = {14.2};
     const Double_t sig[np] = {14.2};
     const Double_t err[np] = {1.8};
...
TGraphErrors* read_E401() {
    //forward data
    
    const int np = 1;
    // (11.292+22.733)/2
    const Double_t w[np]   = {(11.292+22.733)/2-0.2};
    const Double_t sig[np] = {18};
    const Double_t err[np] = {2};
