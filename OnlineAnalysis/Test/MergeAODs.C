...
{
    TFileMerger m;
    m.OutputFile("AliAOD_MC_UD_266025_rapgap_merged.root", "RECREATE");
    
    //int num = GetNumberOfFiles(".", "AliAOD");
    int num = GetNumberOfFiles("allAODs", "AliAOD");
    for (int i = 0; i < num; i++) {
        //TFile* f1 = new TFile(Form("AliAOD-%d.root", i), "READ");
        TFile* f1 = new TFile(Form("allAODs/AliAOD-%d.root", i), "READ");
        m.AddFile(f1);
